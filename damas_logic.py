...
TT_TAMANHO_MB = 128
TT_ENTRIES = (TT_TAMANHO_MB * 1024 * 1024) // 32
TT_FLAG_EXACT = 0
...
class EstadoLanceDesfazer(NamedTuple):
    ...
    foi_promovido: bool
    hash_anterior: int
    damas_recem_promovidas_adicionadas: Set[Posicao] = (
        set()
    )  # Damas adicionadas neste movimento
...
# --- Zobrist Hashing ---
ZOBRIST_TABELA = [
    [[random.randint(1, 2**64 - 1) for _ in range(5)] for _ in range(TAMANHO_TABULEIRO)]
    for _ in range(TAMANHO_TABULEIRO)
]
ZOBRIST_VEZ_PRETA = random.randint(1, 2**64 - 1)
Z_PROM = [
    [random.randint(1, 2**64 - 1) for _ in range(TAMANHO_TABULEIRO)]
    for _ in range(TAMANHO_TABULEIRO)
]
...
# --- Classe Peca ---
class Peca:
    def __init__(self, c: int, t: int = PEDRA):
        self.cor = c
        self.tipo = t

    def __repr__(self) -> str:
        cs = "B" if self.cor == BRANCO else "P"
        ts = "P" if self.tipo == PEDRA else "D"
        return f"{cs}{ts}"

    @staticmethod
    def get_cor(v: int) -> int:
        return VAZIO if v == VAZIO else (BRANCO if v > 0 else PRETO)

    @staticmethod
    def get_tipo(v: int) -> int:
        return VAZIO if v == VAZIO else (DAMA if abs(v) == DAMA else PEDRA)

    @staticmethod
    def get_char(v: int) -> str:
        return {PB: "b", PP: "p", DB: "B", DP: "P"}.get(v, ".")

    @staticmethod
    def get_zobrist_indice(v: int) -> int:
        return {PB: 1, PP: 2, DB: 3, DP: 4}.get(v, 0)
...
class Tabuleiro:
    ...
    def print_all_bitboards(self) -> None:
        ...
        print_bitboard(
            self.bitboard_brancas
            | self.bitboard_damas_brancas
            | self.bitboard_pretas
            | self.bitboard_damas_pretas,
            "Todas as Peças",
        )
    ...
    def _clear_move_and_capture_cache(self) -> None:
        ...
        # self.capturas_cache.clear()  # Removido: não é mais necessário
            ...
            for c in range(8):
                if (r + c) % 2 == 1:  # casas escuras
                    ...
                    if r < 3:
                        ...
                        self.bitboard_brancas |= (
                            b  # Peças brancas nas linhas inferiores
                        )
    ...
    def _atualizar_hash_zobrist(self, r: int, c: int, v: int) -> None:
        if self.is_valido(r, c):
            self.hash_atual ^= ZOBRIST_TABELA[r][c][Peca.get_zobrist_indice(v)]

    def _atualizar_hash_turno(self):
        self.hash_atual ^= ZOBRIST_VEZ_PRETA

    def __repr__(self) -> str:
        h = "  " + " ".join(chr(ord("a") + i) for i in range(TAMANHO_TABULEIRO)) + " "
        l = [h]
        for r in range(TAMANHO_TABULEIRO):
            l.append(
                f"{TAMANHO_TABULEIRO-r:<2}"
                + "".join(
                    Peca.get_char(self.grid[r][c]) + " "
                    for c in range(TAMANHO_TABULEIRO)
                )
                + f"{TAMANHO_TABULEIRO-r:<2}"
            )
    ...
    def mover_peca(self, o: Posicao, d: Posicao) -> None:
        peca = self.get_peca(o)
        self.set_peca(o, VAZIO)
        self.set_peca(d, peca)
        self._clear_move_and_capture_cache()

    def remover_peca(self, p: Posicao) -> None:
        self.set_peca(p, VAZIO)
        self._clear_move_and_capture_cache()

    @staticmethod
    def is_valido(r: int, c: int) -> bool:
        return 0 <= r < TAMANHO_TABULEIRO and 0 <= c < TAMANHO_TABULEIRO

    @staticmethod
    def get_oponente(cor: int) -> int:
    ...
    def limpar_damas_recem_promovidas_por_cor(self, cor: int) -> None:
        """
                ...
                if Peca.get_cor(peca_op) == cor_oponente:
                    ...
                    if self.grid[r_dest][c_dest] == VAZIO:
                        if tipo_op == DAMA or (
                            tipo_op == PEDRA
                            and (
                                (cor_oponente == BRANCO and dr == -1)
                                or (cor_oponente == PRETO and dr == 1)
                            )
                        ):
                            if cache_vulneravel is not None:
                                    ...
                                    if Peca.get_cor(peca_ali) == cor_peca:
                                        ...
                                        if self.grid[r_recapt][c_recapt] == VAZIO:
                                            if tipo_ali == DAMA or (
                                                tipo_ali == PEDRA
                                                and (
                                                    (cor_peca == BRANCO and dr2 == -1)
                                                    or (cor_peca == PRETO and dr2 == 1)
                                                )
                                            ):
                                                return True
        return False
    ...
    def tem_formacao_ponte(self, r: int, c: int) -> bool:
        ...
        if self.is_valido(r, c + 2):
            if (
                self.grid[r][c + 2] != VAZIO
                and Peca.get_cor(self.grid[r][c + 2]) == cor_peca
                and self.grid[r][c + 1] == VAZIO
            ):
                # Para BRANCO, atrás é (r+1, c+1); para PRETO, atrás é (r-1, c+1)
                r_tras = r + 1 if cor_peca == BRANCO else r - 1
                if (
                    self.is_valido(r_tras, c + 1)
                    and Peca.get_cor(self.grid[r_tras][c + 1]) == cor_peca
                ):
                    return True
    ...
    def tem_formacao_lanca(self, r: int, c: int) -> bool:
        ...
        for dc in [-1, 1]:
            ...
            if self.is_valido(r1, c1) and self.is_valido(r2, c2):
                if (
                    self.grid[r1][c1] != VAZIO
                    and Peca.get_cor(self.grid[r1][c1]) == cor_peca
                    and self.grid[r2][c2] == VAZIO
                ):
                    return True
        return False
    ...
    def eh_peca_ameacada_em_2_lances(self, r: int, c: int) -> bool:
        ...
        diagonais_proximas = [
            ...
            (-1, 1),
            (1, -1),
            (1, 1),  # 1 casa de distância
            (-2, -2),
            (-2, 2),
            (2, -2),
            (2, 2),  # 2 casas de distância
        ]
                ...
                if peca != VAZIO:
                    ...
                    if peca_cor == cor_oponente:
                        if Peca.get_tipo(peca) == DAMA:
                            ...
                            if (cor_oponente == PRETO and dr > 0) or (
                                cor_oponente == BRANCO and dr < 0
                            ):
                                atacantes_proximos += 1
    ...
    def get_man_capture_moves_bitboard(
        self, pos: Posicao, cor: int
        ...
        for shift in BB_SHIFTS_ALL:
            ...
            if (
                adjacent_bit != 0
                and (adjacent_bit & oponente) != 0
                and capture_bit != 0
                and (capture_bit & empty) != 0
            ):
                moves |= capture_bit
    ...
    def _search_root(
        self, tab, prof, alpha, beta, movs_ord_raiz, cor_ia, cache_capturas_oponente
    ):
        resultados = {}
        ...
        for mov in movs_ord_raiz:
            if self.verificar_tempo():
                raise TempoExcedidoError(
                    "Tempo excedido durante avaliação de movimentos"
                )
            ...
            capturas_combo = (
                tab.get_all_man_capture_sequences_bitboard(
                    pos_final, cor_ia
                )
                if cp
                else []
            ...
            try:
                ...
                if hash_apos in cache_capturas_oponente:
                    ...
                    capturas_oponente = tab.encontrar_movimentos_possiveis(
                        cor_op, apenas_capturas=True
                    )
                ...
                ct_p = 0
                score = -self.minimax(
                    tab, ct_p, prof - 1, -beta, -alpha, jog_apos, cor_ia, depth=1
                )
                ...
                if self.verificar_tempo():
                    raise TempoExcedidoError(
                        "Tempo excedido após avaliação de um movimento"
                    )
            finally:
    ...
    def can_null_move(self, tab: "Tabuleiro", jog: int, prof: int) -> bool:
        return (
    ...
    def null_move_reduction(self, prof: int) -> int:
        return 2 + prof // 4

    def futility_margin(self, prof: int) -> float:
        return VALOR_PEDRA * prof * 0.3

    def lmr_reduction(self, prof: int, move_index: int) -> int:
        import math

        return (
            1 + int(math.log(prof))
            if move_index >= self.LMR_THRESHOLD and prof > 3
            else 0
        )

    def can_multi_cut(self, prof: int, movs_ordenados: list) -> bool:
        return prof >= 5 and len(movs_ordenados) >= self.MULTICUT_N

    def see(self, tab: "Tabuleiro", pos: Posicao, atacante_cor: int) -> float:
        """
        ...
        def get_all_attackers(tab, pos, cor):
            ...
            for r in range(TAMANHO_TABULEIRO):
                for c in range(TAMANHO_TABULEIRO):
                    peca = tab.grid[r][c]
                    if Peca.get_cor(peca) == cor:
                        movs = tab.encontrar_movimentos_possiveis(
                            cor, apenas_capturas=True
                        )
                        for mov in movs:
                            if mov[-1] == pos:
        ...
        atacantes = {
            BRANCO: get_all_attackers(tab_copia, pos, BRANCO),
            PRETO: get_all_attackers(tab_copia, pos, PRETO),
        }
        idx = 0
        while True:
            ...
            if atacantes[side]:
                next_attacker = min(
                    atacantes[side], key=lambda p: piece_value(tab_copia.get_peca(p))
                )
                gain.append(
                    (-1) ** (idx + 1) * piece_value(tab_copia.get_peca(next_attacker))
                )
            idx += 1
        return sum(gain)

    def get_all_man_capture_sequences_bitboard(
        self,
        pos: Posicao,
        cor: int,
        visited: Optional[Set[Posicao]] = None,
        path: Optional[List[Posicao]] = None,
    ) -> List[List[Posicao]]:
        """
        Gera todas as sequências de capturas múltiplas para uma pedra usando apenas bitboards.
        Retorna uma lista de caminhos (cada caminho é uma lista de posições percorridas).
        """
        if visited is None:
            visited = set()
        if path is None:
        ...
        for shift in BB_SHIFTS_ALL:
            # Descobre a posição da peça a ser capturada e a casa de destino
            if shift == BB_SHIFT_NE:
                adj_bit = (bit_pos >> 7) & BB_NOT_A_FILE
                cap_bit = (adj_bit >> 7) & BB_NOT_A_FILE
            elif shift == BB_SHIFT_NW:
                adj_bit = (bit_pos >> 9) & BB_NOT_H_FILE
                cap_bit = (adj_bit >> 9) & BB_NOT_H_FILE
            elif shift == BB_SHIFT_SE:
                adj_bit = (bit_pos << 9) & BB_NOT_H_FILE
                cap_bit = (adj_bit << 9) & BB_NOT_H_FILE
            elif shift == BB_SHIFT_SW:
                ...
                continue
            # Só segue se existe peça do oponente e casa de destino vazia
            if (
                adj_bit != 0
                and (adj_bit & oponente) != 0
                and cap_bit != 0
                and (cap_bit & empty) != 0
            ):
                pos_adj = self.bit_to_pos(adj_bit)
                pos_cap = self.bit_to_pos(cap_bit)
                if pos_adj in visited or pos_cap in path:
                    continue  # Evita ciclos
                # Marca a peça capturada como visitada
                new_visited = visited | {pos_adj}
                new_path = path + [pos_cap]
                # Simula a captura: remove a peça capturada e move a pedra
                # (Copia o tabuleiro para não afetar o real)
                tab_copia = self.criar_copia()
                tab_copia.remover_peca(pos_adj)
                tab_copia.mover_peca(pos, pos_cap)
                # Busca recursivamente por mais capturas a partir da nova posição
                next_seqs = tab_copia.get_all_man_capture_sequences_bitboard(
                    pos_cap, cor, new_visited, new_path
                )
                if next_seqs:
                    sequences.extend(next_seqs)
                else:
                    sequences.append(new_path)
                found_capture = True
        if not found_capture and len(path) > 1:
            # Se não encontrou mais capturas e já fez pelo menos uma, retorna o caminho atual
            return [path]
        return sequences

    def _validar_capturas_bitboard(self, cor: int) -> bool:
        """
        Valida todas as sequências de capturas (simples e múltiplas) de pedras (man) usando bitboards,
        comparando com o método tradicional. Para cada peça do tipo pedra, compara as sequências de
        capturas possíveis via bitboard com as detectadas pelo método tradicional.
        ...
        for pos in pecas:
            ...
            # Sequências via bitboard
            seqs_bitboard = set(
                tuple(seq)
                for seq in self.get_all_man_capture_sequences_bitboard(pos, cor)
            )
            # Sequências via método tradicional
            seqs_trad = set(
                tuple(mov)
                for mov in self.get_all_man_capture_sequences_bitboard(
                    pos, cor
                )
            )
            if seqs_bitboard != seqs_trad:
                logger.info(f"Divergência de capturas para a peça {pos} cor {cor}:")
                logger.info(f"  Bitboard: {seqs_bitboard}")
                logger.info(f"  Tradicional: {seqs_trad}")
                ok = False
        if ok:
            logger.info(
                f"Validação de capturas (simples e múltiplas) via bitboard OK para cor {cor}."
            )
        else:
            logger.info(
                f"Validação de capturas (simples e múltiplas) via bitboard FALHOU para cor {cor}."
            )
        return ok
    ...
    def criar_copia(self):
        """Cria e retorna uma cópia profunda do tabuleiro atual."""
        return copy.deepcopy(self)

    def encontrar_movimentos_possiveis(self, cor, apenas_capturas=False):
        """
        Retorna todos os movimentos possíveis para a cor informada.
        Se apenas_capturas=True, retorna apenas movimentos de captura.
        Caso contrário, retorna todos os movimentos possíveis (captura e simples).
        """
        movimentos = []
        # Capturas de pedras (man)
        for pos in self.get_posicoes_pecas(cor):
            peca = self.get_peca(pos)
            if Peca.get_tipo(peca) == PEDRA:
                capturas = self.get_all_man_capture_sequences_bitboard(pos, cor)
                for seq in capturas:
                    if len(seq) > 1:
                        movimentos.append(seq)
        # Se só queremos capturas, retornamos aqui
        if apenas_capturas:
            return movimentos
        # Se não há capturas, adiciona movimentos simples
        if not movimentos:
            movimentos_simples = self.gera_movimentos_simples(cor)
            for origem, destino in movimentos_simples:
